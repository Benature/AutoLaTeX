...
# 	ybar = str(ybar)
	...
	for k in range(n):
		...
		b_up += '( {:.2f} - {:.2f} )( {:.2f} - {:.2f} )'.format(xs[k], xbar, ys[k], ybar)
	for k in range(n):
	...
	a_eq = r'{:.2f} - {:.2f} \times {:.2f}'.format(ybar, linear_parameters(xs,ys)[1], xbar)
	
	sigma_y_under = '('
	for k in range(n):
		# sigma_y_under += '[' + str(ys[k]) + '-(' + str(linear_parameters(xs,ys)[1]) + '+' + str(linear_parameters(xs,ys)[0]) +')]^2 +'
		sigma_y_under += '[ {:.2f} -( {:.2f} + {:.2f} )]^2 +'.format(ys[k], linear_parameters(xs,ys)[1], linear_parameters(xs,ys)[0])
...
# 	sigma_b_eq = '\\frac{' + str(linear_parameters(xs,ys)[5])[0:3] + '}{\\sqrt{' + str(n) + sigma_b_under + '}}'
	sigma_b_eq = '\\frac{ %.2f }{\\sqrt{ %.2f %s }}' % (linear_parameters(xs,ys)[5], n, sigma_b_under)
