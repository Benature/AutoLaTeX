...
delete_table_list = ["\\end{minipage}", "\\raggedright", "\\strut", "\\endhead"]
...
def modify_re_table(table):
    ...
    table = re.sub(r"\\begin{minipage}.*?{.*?\\columnwidth}", "", table, flags=re.DOTALL)
    ...
    row_n = list(re.search(r"\n.*?&.*?\n", table, flags=re.DOTALL).group()).count("&")
        ...
        for graphics in figure_li:
            ...
            if len(find_title) == 1:
                ...
                if len(re.findall(r"includegraphics", find_title)) == 1:
                    title = re.findall(r"图 *?\d.*?\n", find_title)[0].replace("\n", "")
                    tex_raw = tex_raw.replace(graphics, template_figure.format(
                              graphics, "{"+title+"}"))
                    tex_raw = tex_raw.replace(title, "{{}}")
                    continue
            tex_raw = tex_raw.replace(graphics, template_figure.format(graphics, "{}"))
